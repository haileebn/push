...
Meteor.methods({
  ...
  'raix:push-setuser': function(id, userID = null) {
    check(id, String);

    if (Push.debug) {
      console.log('Push: Settings userId "' + (userID || this.userId) + '" for app:', id);
    }
    // We update the appCollection id setting the Meteor.userId
    var found = Push.appCollection.update({ _id: id }, { $set: { userId: userID ||this.userId, updatedAt: new Date(),} });
